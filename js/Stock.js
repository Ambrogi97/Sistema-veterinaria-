...
class UI {
  ...
  imprimirCitas() {
    ...
    objectStore.openCursor().onsuccess = function (e) {
      ...
      if (cursor) {
        const { mascota, propietario, telefono, id } =
          cursor.value;
        ...
        propietarioParrafo.innerHTML = `<span class="font-weight-bolder">Cantidad: </span> ${propietario}`;

        const telefonoParrafo = document.createElement("p");
        telefonoParrafo.innerHTML = `<span class="font-weight-bolder">Precio: </span> ${telefono}`;
  ...
  textoHeading(resultado) {
    if (resultado > 0) {
      ...
      heading.textContent = "No hay Prodcutos, comienza creando uno";
    }
  }
...
function cargarEdicion(cita) {
  const { mascota, propietario, telefono, id } = cita;
